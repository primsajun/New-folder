...
const churnedCustomers = data.filter(customer => customer.churned === 1).length;
  ...
  const churnedCustomes = data.filter(customer => customer.churned === 1).length;
...
p
  ...
  return { , normalizedTestData, stats };
...
import * as dfd from 'danfojs-no
  cons
    const normalized = {...it
      // .l
  const churnedCustomers = data.filter(customer => customer.churned === 1).lengt
  const churnRate = churnedCustomers / totalCustomers;

  console.log(`Overall Churn Rate: ${(churnRa  // Calculate churn ra
  ...
  return { normalizedTrainData, normalizedTestData, stats
}  const  = testData.map(item => {   // Calculate churn rate
  const totalCustomers = data.length
  const churnedCustomers = data.filter(customer => customer.churned === 1).length;
  const churnRate = churnedCust
  console.log(`Overall  Rate: ${(churnRate * 100).toFixed(2)}%`)
  
  return { normalizedTrainData, normalizedTestData, stats };
}  const normalizedTestData = testData.map(item => {   // Calculate churn r
  const totalCustomers = data.length;
  const = data.filter(customer => customer.churned === 1).length
  const churnRate =  / totalCustomers;
  
  console.log(`Overall Churn Rate: ${(churnRate * 100).toFixed(2)}%`);
  const churnedCustomers = data.filter(customer => customer.churnedlength;
  const churnRate =  / totalCustomers;
  
  console.log(`Overall Churn Rate: ${(churnRate * 100).toFixed(2)}%`  return { normalizedTrainData, normalizedTestData, stats };
  ...
  return { normalizedTrainData, normalizedTestData, stats };
    ...
    const normali  // Calculate churn r
  const totalCustomers = data.lengt
  const churnedCustomers = data.filter(customer => cu== 1).length;
  const churnRate = churnedCustomers / totalCustomers;
 
  
  return { normalizedTrainData, normalizedTestData, stats };
}  const noconst normalized = {...  // Calculate churn rate
  const totalCustomers = data.length;
  const churnedC.filter(customer => customer.churned === 1).length;
  const churnRate = churnedCustomers / totalCustomers;
